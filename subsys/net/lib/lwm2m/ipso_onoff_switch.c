...
static struct lwm2m_engine_obj_inst *switch_create(u16_t obj_inst_id)
{
	...
	/* Set default values */
	switch_data[avail].trigger_offset = 0U;
	switch_data[avail].on_time_sec = 0U;
	switch_data[avail].off_time_sec = 0U;
	switch_data[avail].counter = 0U;
	switch_data[avail].state = false;
	switch_data[avail].last_state = false;
