...
static struct lwm2m_engine_obj_inst *button_create(u16_t obj_inst_id)
{
	...
	/* Set default values */
	button_data[avail].counter = 0U;
	button_data[avail].state = false;
	button_data[avail].last_state = false;
	...
	INIT_OBJ_RES(BUTTON_DIGITAL_STATE_ID, res[avail], i,
		     ...
		     &button_data[avail].state, sizeof(button_data[avail].state),
...
static int ipso_button_init(struct device *dev)
{
	...
	(void)memset(res, 0, sizeof(struct lwm2m_engine_res) *
			MAX_INSTANCE_COUNT * BUTTON_MAX_ID);
